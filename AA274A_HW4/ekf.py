    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        for ii in range(z_raw.shape[1]):
            ...
            for jj in range(hs.shape[1]):
                v_ij_temp = np.array([angle_diff(z_raw[0, ii], hs[0, jj]), z_raw[1, ii] - hs[1, jj]])
                
                S_ij = np.matmul(np.matmul(Hs[jj], self.Sigma), Hs[jj].T) + Q_raw[ii]
                
                d_ij = np.matmul(np.matmul(v_ij_temp.T, np.linalg.inv(S_ij)), v_ij_temp)
            ...
            if d_ij_min < self.g**2:
                v_list.append(v_ij)
                Q_list.append(Q_raw[ii])
                H_list.append(Hs[jj_min])
...
#        Gu[0:3, 0:3] = Gu_l
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            h, Hx[:, 0:3] = tb.transform_line_to_scanner_frame(np.array([alpha,r]), self.x[0:3], self.tf_base_to_camera)
...
#                Hx[:,idx_j:idx_j+2] = np.eye(2)  # FIX ME!
                ...
                rot_b_to_w = np.array([[np.cos(self.x[2]), -np.sin(self.x[2]), self.x[0]],
