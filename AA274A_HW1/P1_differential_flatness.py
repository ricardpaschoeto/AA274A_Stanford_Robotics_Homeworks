...
def compute_traj_coeffs(initial_state, final_state, tf):
    ...
    """
    ### Respect to state x and xdot
    phi10 = np.array([1,0,0,0,0,0,0,0])
    phi10_dot = np.array([0,1,0,0,0,0,0,0])    
    phiF = np.array([1,tf,tf**2,tf**3,0,0,0,0])
    phiF_dot = np.array([0,1,2*tf,3*tf**2,0,0,0,0])
    
    ### Respect to state y and ydot
    phi20 = np.array([0,0,0,0,1,0,0,0])
    phi20_dot = np.array([0,0,0,0,0,1,0,0])
    phi2F = np.array([0,0,0,0,1,tf,tf**2,tf**3])
    phi2F_dot = np.array([0,0,0,0,0,1,2*tf,3*tf**2])

    Phi = np.array([phi10,phi10_dot,phi20,phi20_dot,phiF,phiF_dot,phi2F,phi2F_dot])
    
    z = np.array([initial_state.x, initial_state.xd, initial_state.y, initial_state.yd,
                  final_state.x  , final_state.xd  , final_state.y  , final_state.yd])

    coeffs = linalg.solve(Phi,z)
    
    # coeffs = [x1,x2,x3,x4,y1,y2,y3,y4]
    if(np.allclose(np.dot(Phi, coeffs), z)):
        return coeffs
...
def compute_traj(coeffs, tf, N):
    ...
    xd = coeffs[1] + 2*coeffs[2]*t + 3*coeffs[3]*(t**2)
    yd = coeffs[5] + 2*coeffs[6]*t + 3*coeffs[7]*(t**2)
    
    traj[:,0] = coeffs[0] + coeffs[1]*t + coeffs[2]*(t**2) + coeffs[3]*(t**3) # x
    traj[:,1] = coeffs[4] + coeffs[5]*t + coeffs[6]*(t**2) + coeffs[7]*(t**3) # y
    traj[:,2] = np.arctan2(yd,xd) # theta
    traj[:,3] = xd
    traj[:,4] = yd
    traj[:,5] = 2*coeffs[2] + 6*coeffs[3]*t # xdd
    traj[:,6] = 2*coeffs[6] + 6*coeffs[7]*t # ydd
...
def compute_controls(traj):
    ...
    """
    V = (traj[:,3]**2 + traj[:,4]**2)**(0.5)
    om = ((traj[:,6]*traj[:,3] - traj[:,4]*traj[:,5])/(V*np.cos(traj[:,2]))**2)*(np.cos(traj[:,2])**2)
...
def rescale_V(V, om, V_max, om_max):
    ...
    """
    if(np.abs((np.clip(om, -om_max, om_max)/om)*V).any() > V_max):
        V_tilde = np.clip(((np.clip(om, -om_max, om_max))/om)*V, -V_max, V_max)
