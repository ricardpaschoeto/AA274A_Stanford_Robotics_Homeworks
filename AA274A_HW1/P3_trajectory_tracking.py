...
class TrajectoryTracker:
    ...
    def compute_control(self, x, y, th, t):
        ...
        if(self.V_prev <= V_PREV_THRES):
            self.V_prev = (yd_d**2 + xd_d**2)**(0.5)
            
        J = np.array([[np.cos(th),-self.V_prev*np.sin(th)],[np.sin(th),self.V_prev*np.cos(th)]])
        U = np.array([xdd_d + self.kpx*(x_d - x) + self.kdx*(xd_d - xdot),
                      ydd_d + self.kpy*(y_d - y) + self.kdy*(yd_d - ydot)])
        
        inputs = linalg.solve(J,U)
        V = self.V_prev + inputs[0]*dt
        om = self.om_prev + inputs[1]
