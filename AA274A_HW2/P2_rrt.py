...
class RRT(object):
    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        for k in range(max_iters):
            ...
            ii_near = self.find_nearest(V[range(n),:], x_rand)
            x_near = V[ii_near,:]
            ...
            if self.is_free_motion(self.obstacles, x_near, x_new):
                ...
                P[n] = ii_near
                n = n+1
                if (int(x_new[0]) == self.x_goal[0] and int(x_new[1]) == self.x_goal[1]):
                    success = True
                    break
        
        if success:
            self.reconstruct_path(V[range(n),:],P[range(n)], n)
    ...
    def shortcut_path(self):
        ...
        while not success:
            ...
            l = len(self.path)
            while (ii < (l-1)):
                if np.all(self.path[ii] != self.x_init) and np.all(self.path[ii] != self.x_goal): 
